...
import type { Delinquent } from '@/data/finance/delinquencyData';
...
/** Bloco de Inadimplência (mensalidades vencidas e não pagas). */
          ...
          <tbody>
            {rows.map((d) => (
              <tr key={d.id} className="border-b border-border last:border-0 hover:bg-canvas">
                <td className="px-5 py-2.5 font-medium text-ink">{d.studentName}</td>
                <td className="hidden px-5 py-2.5 text-ink-muted lg:table-cell">{d.guardianName}</td>
                <td className="hidden px-5 py-2.5 text-ink-muted sm:table-cell">{d.plan}</td>
                <td className="whitespace-nowrap px-5 py-2.5 text-ink-muted">{fmtDate(d.dueDate)}</td>
                <td className="px-5 py-2.5 text-center font-semibold text-danger">{d.daysLate}d</td>
