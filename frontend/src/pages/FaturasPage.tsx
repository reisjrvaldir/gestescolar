    ...
    <>
      ...
      {invoices.length === 0 ? (
        ...
        <div className="space-y-6">
          {/* 4 Cards — 70% width grid */}
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4" style={{ maxWidth: '70%' }}>
            <div className="card p-5">
              <p className="text-sm font-medium text-ink-muted">Mensalidades em aberto</p>
              <p className="mt-1 text-2xl font-extrabold text-warning">{brl(currentMonthPending.reduce((s, i) => s + i.amount, 0))}</p>
              <p className="mt-1 text-xs text-ink-subtle">{currentMonthPending.length} fatura(s)</p>
            </div>
            <div className="card p-5">
              <p className="text-sm font-medium text-ink-muted">Pagas</p>
              <p className="mt-1 text-2xl font-extrabold text-success">{brl(paidTotal)}</p>
              <p className="mt-1 text-xs text-ink-subtle">{paidInvoices.length} fatura(s)</p>
            </div>
            <div className="card p-5">
              <p className="text-sm font-medium text-ink-muted">Vencidas</p>
              <p className="mt-1 text-2xl font-extrabold text-danger">{brl(overdueInvoices.reduce((s, i) => s + i.amount, 0))}</p>
              <p className="mt-1 text-xs text-ink-subtle">{overdueInvoices.length} fatura(s)</p>
            </div>
            <div className="card p-5">
              <p className="text-sm font-medium text-ink-muted">Total do mês</p>
              <p className="mt-1 text-2xl font-extrabold text-ink">{brl(totalMes)}</p>
              <p className="mt-1 text-xs text-ink-subtle">Mensalidade + avulsas</p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 gap-6 lg:grid-cols-[7fr_3fr]">
            ...
            <div className="space-y-6">
              ...
              <div className="card overflow-hidden">
                ...
                {invoices.length === 0 ? (
                  <p className="px-5 py-8 text-center text-sm text-ink-subtle">Nenhuma fatura.</p>
                ) : (
                  <div className="divide-y divide-border">
                    {invoices
            ...
            <div>
              {selected ? (
                ...
                <div className="card p-8 text-center text-ink-subtle">
                  ...
                  <p className="text-sm">Selecione uma fatura ao lado para ver os detalhes e opções de pagamento.</p>
                </div>
              )}
            </div>
          </div>
