  ...
  editSchool(schoolId) {
    ...
    Utils.modal(`Editar – ${Utils.escape(school.name)}`,
      `<form onsubmit="SuperAdmin.saveSchool(event,'${schoolId}')">
        ...
        <!-- SEÇÃO ASAAS -->
        <div style="background:var(--bg-secondary);border-radius:8px;padding:16px;margin:16px 0;border-left:4px solid var(--secondary);">
          <div style="font-weight:600;margin-bottom:12px;font-size:14px;">
            <i class="fa-solid fa-building-columns"></i> Gateway de Pagamentos (Asaas)
          </div>
          ...
          ` : `
            <div style="margin-bottom:12px;padding:12px;background:#d4edda;border-radius:6px;font-size:13px;color:#155724;">
              <i class="fa-solid fa-check-circle"></i> <strong>Subconta configurada.</strong>
        ...
        <div class="form-group">
          ...
          <input class="form-control" id="esWalletId" value="${Utils.escape(school.asaasWalletId||'')}" placeholder="Preenchido ao criar subconta Asaas" readonly style="background:#f0f0f0;" />
          <small style="color:var(--text-muted);font-size:11px;">ID da carteira Asaas para split automático de pagamentos. Preenchido automaticamente.</small>
        </div>
        <div class="form-group">
...
supabase secrets set SUPABASE_SERVICE_ROLE_KEY=&lt;service-role-key&gt;</code>
  ...
  async _confirmDelete(schoolId) {
    ...
    // Obter token JWT do superadmin para autenticar no proxy
    const { data: sessionData } = await supabaseClient.auth.getSession().catch(() => ({ data: null }));
    const token = sessionData?.session?.access_token;
    if (!token) { Utils.toast('Sessão expirada. Faça login novamente.', 'error'); return; }

    // Excluir via /api/admin com service key (bypass de RLS)
    // Garante exclusão mesmo que auth_id não esteja vinculado na tabela users
    let result;
    try {
      const res = await fetch('/api/admin', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` },
        body: JSON.stringify({ action: 'deleteSchool', data: { schoolId } }),
      });
      result = await res.json();
      if (!res.ok) throw new Error(result.error || `HTTP ${res.status}`);
    } catch (err) {
      console.error('[SuperAdmin] Erro ao excluir escola:', err);
      Utils.toast(`Erro ao excluir escola: ${err.message}`, 'error');
      return;
    }

    // Só remove da cache LOCAL após confirmação do servidor
